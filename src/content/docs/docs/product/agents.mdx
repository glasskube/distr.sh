...
<Aside type="note">
  Unless you're using Distr to deploy your own applications internally, the Distr agent will always be configured and deployed by the end customer. Head over to the [end-customer agent guide](/docs/guides/end-customer-registry-view/#deployment-agent-quickstart-from-the-customer-portal) to learn how it should be installed.
</Aside>
...
- **🔴 Error (Red)**  
  ...
  ➤ *New deployments cannot be created until the error state is resolved.*

- **🟡 Stale (Yellow)**  
  The agent has not made a `resource` request to Distr Hub within the last 60 seconds.  
  ➤ *This indicates a communication issue. No new deployments can be made until the agent is healthy.*

- **🟢 OK (Green)**  
  The agent is actively communicating with Distr Hub and reporting a healthy application state.  
  ➤ *New deployments are allowed. The **+ Deployment** button will be visible.*
