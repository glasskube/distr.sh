...
<Aside type="tip">
  Use descriptive names for your secret keys that clearly indicate their purpose,
  such as `API_KEY_GITHUB` or `DATABASE_PASSWORD_PROD`.
</Aside>
...
<Aside type="caution">
  Secret values cannot be empty. If you need to represent an empty value, consider
  using a placeholder string or removing the secret reference entirely.
</Aside>
